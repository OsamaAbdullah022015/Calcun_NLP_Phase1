...
class GraphPlotter(API):
    def __init__(self, dtf, baseURL):
        ...
        self.dtf = dtf
        self.display_graph = API('http://localhost:6000')
    ...
    def plot_entity_based_graphs(self, word):
        # payload = {
        #     'dtf' : self.dtf.to_dict(),
        #     'entity' : word
        #            }
        # self.display_graph.post('show-on-streamlit',  payload)
        self.plot_graph(word)
    ...
    def saving_kg_as_json(self, entity):
        ...
        with open(f"./output/{result}.json", "w") as outfile: 
